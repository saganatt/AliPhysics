...
struct TrackDeriv {
  ...
  TrackDeriv() = default;
  TrackDeriv(const o2::track::TrackParCov& trc, float bz) { set(trc, bz); }
  void set(const o2::track::TrackParCov& trc, float bz)
  {
...
bool DCAFitterN<N, Args...>::minimizeChi2()
{
  ...
  do {
    ...
    if (!mD2Chi2Dx2.Invert()) {
      std::cerr << "InversionFailed" << std::endl;
      return false;
    }
...
{
  std::cout << N << "-prong vertex fitter in " << (mUseAbsDCA ? "abs." : "weighted") << " distance minimization mode" << std::endl;
  std::cout << "Bz: " << mBz << " MaxIter: " << mMaxIter << " MaxChi2: " << mMaxChi2 << std::endl;
  std::cout << "Stopping condition: Max.param change < " << mMinParamChange << " Rel.Chi2 change > " << mMinRelChi2Change << std::endl;
